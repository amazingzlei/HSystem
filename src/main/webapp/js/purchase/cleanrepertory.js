$(function () {
    $.ajax(
        {
            url:urlPath+'/purchaseInfo/getDrawMed',
            type:'GET',
...
function showPageEmployeeData(data) {
    ...
    html += "<td>" +
            "<a href='#' class='btn btn-danger btn-xs' onclick='cleanRepertory(this)'>清仓</a></tr>";
    $("tbody").append(html);
}

function cleanRepertory(obj) {
    var mid = $(obj).parent().parent().children().eq(0).text();
    var repertoryId = $(obj).parent().parent().children().eq(2).text();
    var productTime = $(obj).parent().parent().children().eq(4).text();
    $.ajax({
        url:urlPath+"/purchaseInfo/cleanRepertory",
        type:"POST",
        data:{mid:mid,repertoryId:repertoryId,productTime:productTime},
        success:function (data) {
            if(data.code == 200){
                toastr.success("清仓成功!");
