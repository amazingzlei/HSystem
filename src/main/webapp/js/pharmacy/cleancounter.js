$(function () {
    $.ajax(
        {
            url:urlPath+'/pharmacyInfo/getDrawMed',
            type:'GET',
...
function showPageEmployeeData(data) {
    ...
    html += "<td>" +
            "<a href='#' class='btn btn-danger btn-xs' onclick='cleanCounter(this)'>清柜</a></tr>";
    $("tbody").append(html);
}

function cleanCounter(obj) {
    var mid = $(obj).parent().parent().children().eq(0).text();
    var counterId = $(obj).parent().parent().children().eq(2).text();
    var productTime = $(obj).parent().parent().children().eq(4).text();
    $.ajax({
        url:urlPath+"/pharmacyInfo/cleanCounter",
        type:"POST",
        data:{mid:mid,counterId:counterId,productTime:productTime},
        success:function (data) {
            if(data.code == 200){
                toastr.success("清柜成功!");
